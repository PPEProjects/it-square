...
// Filter projects
const filter = reactive<GetProjectsVariables>({
  filter: {
    limit: 7,
    offset: 0,
    sort: 'createdAt'
...
watch(
  filter,
  () => {
    const _filter: any = Object.assign({}, filter.filter)
...
const { result: countResult } = useQuery<
  CountProjects,
  CountProjectsVariables
...
const onChangePage = (page: number) => {
  Object.assign(filter.filter, {
    offset: (page - 1) * filter.filter.limit
  })
}
</script>
